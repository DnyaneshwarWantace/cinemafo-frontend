...
const MovieModal: React.FC<MovieModalProps> = ({ movie: initialMovie, onClose }) => {
  ...
  const fetchTrailer = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://24.144.84.120/api/trailer/${movieId}`);
  ...
  const changeQuality = async (quality: any) => {
    ...
    try {
      ...
      const response = await fetch(`http://24.144.84.120/api/trailer/${movie.id}/quality/${quality.formatId}`);
  ...
  const downloadTrailer = async (quality?: any) => {
    ...
    try {
      ...
      const response = await fetch(`http://24.144.84.120/api/trailer/${movie.id}/download/${formatId}`);
