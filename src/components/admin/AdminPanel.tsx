...
const adminApi = {
  login: async (username: string, password: string) => {
    const response = await fetch(`${import.meta.env.VITE_BACKEND_URL || 'https://cinema.bz/api'}/admin/login`, {
      method: 'POST',
  ...
  refreshToken: async () => {
    ...
    const response = await fetch(`${import.meta.env.VITE_BACKEND_URL || 'https://cinema.bz/api'}/admin/refresh-token`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  getSettings: async () => {
    ...
    const response = await apiCallWithRefresh(() => 
      fetch(`${import.meta.env.VITE_BACKEND_URL || 'https://cinema.bz/api'}/admin/settings`, {
  ...
  updateAnnouncement: async (settings: any) => {
    const token = localStorage.getItem('adminToken');
    const response = await fetch(`${import.meta.env.VITE_BACKEND_URL || 'https://cinema.bz/api'}/admin/settings/announcement`, {
      method: 'PUT',
      headers: { 
  ...
  updateSocialButtons: async (settings: any) => {
    const token = localStorage.getItem('adminToken');
    const response = await fetch(`${import.meta.env.VITE_BACKEND_URL || 'https://cinema.bz/api'}/admin/settings/social-buttons`, {
      method: 'PUT',
      headers: { 
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
  ...
  updateSocialLinks: async (socialLinks: any) => {
    const token = localStorage.getItem('adminToken');
    const response = await fetch(`${import.meta.env.VITE_BACKEND_URL || 'https://cinema.bz/api'}/admin/settings/social-links`, {
      method: 'PUT',
      headers: { 
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
  ...
  updateContent: async (contentData: any) => {
    ...
    const response = await fetch(`${import.meta.env.VITE_BACKEND_URL || 'https://cinema.bz/api'}/admin/settings/content`, {
      method: 'PUT',
      headers: { 
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
  ...
  updateAds: async (settings: any) => {
    const token = localStorage.getItem('adminToken');
    const response = await fetch(`${import.meta.env.VITE_BACKEND_URL || 'https://cinema.bz/api'}/admin/settings/ads`, {
      method: 'PUT',
      headers: { 
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
  ...
  uploadAdImage: async (adKey: string, imageUrl: string) => {
    const token = localStorage.getItem('adminToken');
    const response = await fetch(`${import.meta.env.VITE_BACKEND_URL || 'https://cinema.bz/api'}/admin/upload-ad-image`, {
      method: 'POST',
      headers: { 
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
  ...
  updateCSS: async (css: string) => {
    const token = localStorage.getItem('adminToken');
    const response = await fetch(`${import.meta.env.VITE_BACKEND_URL || 'https://cinema.bz/api'}/admin/settings/css`, {
      method: 'PUT',
      headers: { 
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
...
const AdAnalyticsDashboard: React.FC = () => {
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL || 'https://cinema.bz/api'}/admin/admin/ad-analytics?days=${timeRange}${selectedAd ? `&adKey=${selectedAd}` : ''}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const fetchSummary = async () => {
    try {
      const token = localStorage.getItem('adminToken');
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL || 'https://cinema.bz/api'}/admin/admin/ad-summary?days=${timeRange}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const exportData = async (format: 'json' | 'csv' = 'json') => {
    try {
      const token = localStorage.getItem('adminToken');
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL || 'https://cinema.bz/api'}/admin/admin/export-ad-data?format=${format}${selectedAd ? `&adKey=${selectedAd}` : ''}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
